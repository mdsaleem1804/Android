...
public class DataBaseConnection extends SQLiteOpenHelper {
	...
	public void onCreate(SQLiteDatabase db) {
		try {
			...
			xUserTable = "create table userdetails "
					+ "(usermobileno varchar(50),userplace varchar(50))";
			db.execSQL(xUserTable);

			xUserSendRequest = "create table usersendrequest "
					+ "(usersrmobileno varchar(50),usersplace varchar(50),usersrdrivername varchar(50))";
	...
	public void insertUserDetails(String xUserMobileNo,String xUserPlace) {

		try {

			ContentValues contentValues = new ContentValues();
			contentValues.put("usermobileno", xUserMobileNo);
			contentValues.put("userplace", xUserPlace);			
	...
	public boolean fn_GetDriverRegistration(String xLicenceNo) {

		boolean result = false;
		try {
			String query = "SELECT  * FROM driverregistration where licenceno='"
					+ xLicenceNo + "';";
	...
	public void insertUserSendRequest(String xUserMobileNo,String xUsersPlace, String xDriverName) {

		try {
			ContentValues contentValues = new ContentValues();
			contentValues.put("usersrmobileno", xUserMobileNo);
			contentValues.put("usersrdrivername", xDriverName);
			contentValues.put("usersplace", xUsersPlace);
	...
	public List<String> fn_GetUserRequests() {
		List<String> xUserRequests = new ArrayList<String>();
		SQLiteDatabase db = this.getWritableDatabase();
		String xQry = "Select * from usersendrequest";
		try {
			Cursor cursor = db.rawQuery(xQry, null);
			...
			if (cursor.moveToFirst()) {
				do {
					xUserRequests.add(cursor.getString(0)+"-"+cursor.getString(1));
				} while (cursor.moveToNext());
			}
			cursor.close();
			db.close();
