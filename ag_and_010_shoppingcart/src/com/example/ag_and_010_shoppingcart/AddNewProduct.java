...
public class AddNewProduct extends Activity {
	...
	EditText xEdtAddProductName, xEdtAddProductPrice, xEdtAddProductQty,
			xEdtAddProductDescription;
	String xVendorName = "";
	...
	public void addproduct(View v) {
		mCon.fn_InsertProductDetails(xEdtAddProductName.getText().toString(),
				xEdtAddProductPrice.getText().toString(), xEdtAddProductQty
						.getText().toString(), xVendorName, xByteImage,
				xEdtAddProductDescription.getText().toString());
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		 ...
		 * switch (requestCode) {
		 * 
		 * case 1: if (resultCode == RESULT_OK) { Bitmap photo = (Bitmap)
		 * data.getExtras().get("data"); // Drawable d = new
		 * BitmapDrawable(getResources(), photo); ByteArrayOutputStream bos =
		 * new ByteArrayOutputStream();
		 * photo.compress(Bitmap.CompressFormat.PNG, 100, bos); xByteImage =
		 * bos.toByteArray(); xProductImg.setImageBitmap(photo);
		 * 
		 * } break; }
		 */
