...
public class DataBaseConnection extends SQLiteOpenHelper {
	...
	public void onCreate(SQLiteDatabase db) {
		try {
			...
			xProduct = "create table product "
					+ "(pname varchar(50),pprice varchar(50),pqty int(5),pvendorname varchar(50)," +
					"image blob,pdescription varchar(200))";
			db.execSQL(xProduct);
	...
	public Boolean fn_UpdateProduct(String xProductName, int xQty) {

		try {

			ContentValues contentValues = new ContentValues();
			contentValues.put("pqty", xQty);
			SQLiteDatabase db = this.getWritableDatabase();
			db.update("product", contentValues, "pname='" + xProductName + "'",
					null);
	...
	public Boolean fn_InsertProductDetails(String xProductName,
			String xProductPrice, String xProductQty, String xVendorName,
			byte[] xByteImage,String xProductDescription) {
		...
		try {
			...
			contentValues.put("pqty", xProductQty);
			contentValues.put("pvendorname", xVendorName);
			contentValues.put("image", xByteImage);
			contentValues.put("pdescription", xProductDescription);
	...
	public Boolean fn_InsertCart(String xProductName, String xPrice,
			String xVendorName, String xUserName) {

		try {
			ContentValues contentValues = new ContentValues();
			contentValues.put("pname", xProductName);
			contentValues.put("pprice", xPrice);
			contentValues.put("pvendorname", xVendorName);
	...
	public List<String> fn_GetVendors(String xGetFloorName) {
		List<String> xBus = new ArrayList<String>();
		SQLiteDatabase db = this.getWritableDatabase();
		String xQry = "Select * from vendorregistration where floor='"
				+ xGetFloorName + "'";
		try {
	...
	public List<String> fn_GetProducts(String xVendorName) {
		List<String> xBus = new ArrayList<String>();
		SQLiteDatabase db = this.getWritableDatabase();
		String xQry = "Select * from product where pvendorname ='"
				+ xVendorName + "'";
		try {
			Cursor cursor = db.rawQuery(xQry, null);
			// xExpenses.add("LI")
			...
			if (cursor.moveToFirst()) {
				do {
					xBus.add(cursor.getString(0) + "-" + cursor.getString(1)
							+ "-" + cursor.getString(2));
				} while (cursor.moveToNext());
			}
	...
	public List<Contact> getAllProducts(String xVendorName) {
		...
		// Select All Query
		String selectQuery = "SELECT  * FROM product where pvendorname ='"
				+ xVendorName + "'";
	...
	public List<String> fn_GetCart(String xVendorName) {
		...
		try {
			...
			if (cursor.moveToFirst()) {
				do {
					xCartItem.add("Pr.Name : " + cursor.getString(0)
							+ "Price : " + cursor.getString(1) + "User : "
							+ cursor.getString(3));
				} while (cursor.moveToNext());
			}
	...
	public List<String> fn_GetStock(String xVendorName) {
		List<String> xCartItem = new ArrayList<String>();
		SQLiteDatabase db = this.getWritableDatabase();
